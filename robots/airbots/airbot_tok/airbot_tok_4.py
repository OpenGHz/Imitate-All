...
from robots.airbots.airbot_play.airbot_play_4 import AIRBOTPlayConfig, AIRBOTPlay
...
class AIRBOTTOK(object):
    def __init__(self, config: Optional[AIRBOTTOKConfig] = None, **kwargs) -> None:
        ...
        self._exited = False
        logger.info("TOK started")

    def __init(self):
        logger.info("TOK initiating")
        # Connect the cameras
    ...
    def reset(self):
        logger.info("TOK resetting")
        for name, arm in self.arms.items():
            logger.info(f"Reseting arm {name}")
            arm.reset()
        self._state_mode = "active"
    ...
    def send_action(self, action, wait=False):
        ...
        for index, arm in enumerate(self.arms.values()):
            arm.send_action(arm_targets[index], wait)
        return action
    ...
    def get_low_dim_data(self):
        ...
        for arm_name, arm in self.arms.items():
            ...
            for key, value in low_dim.items():
                data[key.replace("observation", f"observation/{arm_name}")] = value
        if self.base is not None:
