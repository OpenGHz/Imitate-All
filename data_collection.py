...
def parse_args()-> argparse.Namespace:
    ...
    parser.add_argument("--leader-domain-id", type=list[int], default=[100], help="Domain id of the leader")
    parser.add_argument("--follower-domain-id", type=list[int], default=[99], help="Domain id of the follower")
...
class data_collection:
    def __init__(self, args):
        self.args = args
        self.leader_robot, self.follower_robot = self.init_robot()
        self.data = []
        
    def init_robot(self)-> list[list]:
        args = self.args
        leader_robot = []
        follower_robot = []
        ...
        return leader_robot, follower_robot
    ...
    def reset_robot(self):
        ...
        for i in range(args.follower_number):
            ...
            assert follower_robot[i].online_traj_mode(), "Follower robot %d online traj mode failed" % i
        for i in range(args.leader_number):
            if args.leader_arm_type[i] == "replay":
                continue
            assert leader_robot[i].set_target_joint_q(args.start_joint_position), "Leader robot %d set target joint q failed" % i
        for i in range(args.follower_number):
            assert follower_robot[i].set_target_joint_q(args.start_joint_position), "Follower robot %d set target joint q failed" % i
            assert follower_robot[i].set_target_end(args.start_end_effector_position), "Follower robot %d set target end effector position failed" % i
        for i in range(args.leader_number):
            if args.leader_arm_type[i] == "replay":
                continue
            assert leader_robot[i].online_idle_mode(), "Leader robot %d online idle mode failed" % i
        for i in range(args.follower_number):
            ...
            while follower_robot[i].get_current_state() != "SLAVE_REACHING":
                time.sleep(0.1)
    ...
    def start_data_collection(self, round:int):
        ...
        for i in range(args.leader_number):
            if args.leader_arm_type[i] == "replay":
                continue
            assert self.leader_robot[i].demonstrate_start_mode(), "Leader robot %d demonstrate start mode failed" % i
