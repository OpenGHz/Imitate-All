...
if __name__ == "__main__":
    ...
    image_data: Dict[str, np.ndarray] = read_sub_data_from_hdf5(
        root_raw, data_structure
    )
    ...
    print(
        img_padded.shape
    )  # (1, 200, 1xxxx), 1xxxx is the max length of compressed image
    root_raw.close()
    # save data to target hdf5 files
    max_timesteps = img_padded.shape[1]
    print("Saving augmented data to hdf5 file...")
    target_file_path = (
        "/home/ghz/Work/ALOHA/act/data/hdf5/stack_cups/episode_0_augmented.hdf5"
    )
    if target_file_path != raw_file_path:
        ...
        print(
            "Warning: target file path is the same as raw file path, data will be overwritten."
        )
    try:
