...
class RawDataset(object):
    ...
    def warm_up_episodes(self, episodes: List[int]):
    ...
    def load_episode(self, episode):
        ...
        # load the meta.json
        with open(self.data_path / episode / "meta.json", "r") as f:
            meta = json.load(f)
            self.raw_data[episode]["meta"] = meta
        # load the low_dim.json
        with open(self.data_path / episode / "low_dim.json", "r") as f:
            low_dim = json.load(f)
            self.raw_data[episode]["low_dim"] = low_dim
        # check the video files
        video_files = list((self.data_path / episode).glob("*.mp4")) + list(
    ...
    def select_columns(self, keys: Union[str, List[str]]) -> List[dict]:
        if isinstance(keys, str):
            keys = [keys]
        items: List[dict] = []
        for episode, value in self.raw_data.items():
